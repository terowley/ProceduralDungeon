...
public class Room
{
	...
	private List<Vector2Int> FindFreeRegion (Vector2Int sizeInTiles) {
		...
		do {
			region.Clear();
			// CHANGED RANGE TO LEAVE CLEARANCE IN FRONT OF DOORS
			Vector2Int centerTile = new Vector2Int(UnityEngine.Random.Range(3, 18 - 3), UnityEngine.Random.Range(3, 10 - 3));
	...
	public void AddPopulationToTilemap (Tilemap tilemap, TileBase obstacleTile) {
		...
		string heading = PlayerMovement.heading;

		GameObject player = GameObject.FindGameObjectWithTag("Player");



		//Debug.Log("in room heading " + heading);

		if (heading == "N")
		{
			//this.population[8, 1] = "Player";
			player.transform.position = new Vector2(-.45f, -3.5f);

		}
		if (heading == "S")
		{
			//this.population[8, 8] = "Player";
			player.transform.position = new Vector2(-.45f, 3.5f);
		}
		if (heading == "E")
		{
			
			//this.population[1, 5] = "Player";
			player.transform.position = new Vector2(-7.5f, 0.5f);
		}
		if (heading == "W")
		{
			//this.population[16, 5] = "Player";
			player.transform.position = new Vector2(7.5f, 0.5f);
		}

		//GameObject player = GameObject.Find("Player");
		//this.name2Prefab["Player"] = player;
